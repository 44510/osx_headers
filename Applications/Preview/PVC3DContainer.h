//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "PVMediaContainerBase.h"

@class NSDictionary, NSMutableArray, PVC3DThumbnailRenderer, SCNRenderer, SCNScene, SCNSceneSource;

@interface PVC3DContainer : PVMediaContainerBase
{
    NSMutableArray *_elements;
    SCNSceneSource *_sceneSource;
    SCNScene *_scene;
    PVC3DThumbnailRenderer *_cameraSharedThumbnailRenderer;
    NSDictionary *_properties;
    BOOL _viewedReadOnlyWarning;
}

@property(readonly, retain) NSDictionary *properties; // @synthesize properties=_properties;
@property(readonly, retain) SCNSceneSource *sceneSource; // @synthesize sceneSource=_sceneSource;
@property(readonly, retain) SCNScene *scene; // @synthesize scene=_scene;
- (void)cancelWriting;
- (BOOL)canCancelWriting;
- (BOOL)writePDFToURL:(id)arg1 withOptions:(id)arg2 delegate:(id)arg3;
- (BOOL)writeElementToMovie:(id)arg1 toURL:(id)arg2 withSize:(struct CGSize)arg3 delegate:(id)arg4;
- (BOOL)writeElementsAtIndexes:(id)arg1 asImageType:(id)arg2 toURL:(id)arg3 withSize:(struct CGSize)arg4;
- (BOOL)writeElementsAtIndexes:(id)arg1 asImageType:(id)arg2 toURL:(id)arg3 withOptions:(id)arg4 delegate:(id)arg5;
- (BOOL)writeToURL:(id)arg1 ofType:(id)arg2 withOptions:(id)arg3 delegate:(id)arg4;
- (id)bestTypeForWriting;
- (id)writableTypesForSaveOperation:(unsigned long long)arg1;
- (BOOL)writeToURL:(id)arg1 ofType:(id)arg2 error:(id *)arg3;
- (id)pageContainer;
- (BOOL)isAnnotationLeaf;
- (unsigned long long)childAnnotationCount;
- (void)removeObjectFromElementsAtIndex:(unsigned long long)arg1;
- (void)insertObject:(id)arg1 inElementsAtIndex:(unsigned long long)arg2;
- (unsigned long long)indexOfElement:(id)arg1;
- (id)elementAtIndex:(unsigned long long)arg1;
- (id)elements;
- (id)aggregateKeywords;
- (unsigned long long)elementCount;
- (struct CGImageSource *)source;
- (id)fileAttributes;
- (BOOL)validateSource:(id *)arg1;
- (id)bookmarks;
- (void)removeAnnotationsAtIndexes:(id)arg1;
- (void)insertAnnotations:(id)arg1 atIndexes:(id)arg2;
- (struct _NSRange)indexRangeOfAnnotationsForElement:(id)arg1;
- (id)annotations;
- (void)ensureAnnotationsAreLoaded;
- (id)documentAttributes;
- (BOOL)isContentLoadingContainer;
- (BOOL)isQuickLookContainer;
- (BOOL)isC3DContainer;
- (BOOL)isImageContainer;
- (BOOL)isPDFContainer;
- (id)authoringTool;
@property(readonly, retain) SCNRenderer *cameraSharedThumbnailRenderer;
- (void)_clearSharedThumbnailRenderer;
- (void)updateChangeCount:(unsigned long long)arg1;
- (BOOL)checkAutosavingSafetyAndReturnError:(id *)arg1;
- (void)dealloc;
- (id)initWithURL:(id)arg1 ofType:(id)arg2 windowController:(id)arg3 options:(id)arg4 validate:(BOOL)arg5 error:(id *)arg6;

@end

