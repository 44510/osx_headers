//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class ConversationMember, MCActivityMonitor, MUIWebDocument;

@interface WebDocumentGenerator : NSObject
{
    MUIWebDocument *_webDocument;
    ConversationMember *_conversationMember;
    MCActivityMonitor *_loadMonitor;
    CDUnknownBlockType _asyncLoadCompletionHandler;
}

+ (id)_loadQueue;
@property(copy, nonatomic) CDUnknownBlockType asyncLoadCompletionHandler; // @synthesize asyncLoadCompletionHandler=_asyncLoadCompletionHandler;
@property(retain, nonatomic) MCActivityMonitor *loadMonitor; // @synthesize loadMonitor=_loadMonitor;
@property(retain, nonatomic) ConversationMember *conversationMember; // @synthesize conversationMember=_conversationMember;
@property(retain, nonatomic) MUIWebDocument *webDocument; // @synthesize webDocument=_webDocument;
- (void).cxx_destruct;
- (void)cancelLoad;
- (void)_showLoadingProgress;
- (void)asyncLoad:(CDUnknownBlockType)arg1;
- (id)_dataDetectorsContextForMessage:(id)arg1;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (void)dealloc;
- (id)initWithConversationMember:(id)arg1;

@end

